...
snprintf(ts, 32, "%" PRIdMAX ".%06ld", (intmax_t)fb->timestamp.tv_sec, fb->timestamp.tv_usec);
...
static esp_err_t command_handler(httpd_req_t *req) {
  ...
  if (buf_len > 1 && buf_len < sizeof(buf)) {
    ...
    char cmdRaw[10] = {0};
    if (httpd_query_key_value(buf, "cmd", cmdRaw, sizeof(cmdRaw)) == ESP_OK) {
      String cmd = String(cmdRaw);  // Перетворення в String
      String res = handleCommand(cmd);           // Виведення через функцію

      return httpd_resp_send(req, res.c_str(), HTTPD_RESP_USE_STRLEN);
    } 
  }
